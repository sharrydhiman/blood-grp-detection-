...
if data['training_data'].shape[0] == 0 or data['training_labels'].shape[0] == 0:
    ...
    adaboost_model.fit(data['training_data'], data['training_labels'])

    # Test performance of AdaBoosted model
    y_pred_adaboost = adaboost_model.predict(data['validation_data'])
    ...
    for blood_group in blood_group_labels:
        ...
        y_pred_adaboost_blood_group = adaboost_model.predict(validation_data_blood_group)
    ...
    print('\nClassification Report - AdaBoosted:')
    ...
    cmd_adaboost.plot()
    plt.title('Confusion Matrix - AdaBoosted')
    plt.show()

    # Saving AdaBoosted model to desired location
    #save_path_adaboost = r"C:\Users\sharr\Desktop\SMLPROJECT\adaboost_model.pkl"
    #with open(save_path_adaboost, 'wb') as f:
     #   pickle.dump(adaboost_model, f)

    #print(f"AdaBoosted model saved successfully as '{save_path_adaboost}'")
